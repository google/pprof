...
func (bu *Binutils) Open(name string, start, limit, offset uint64, baseName string) (plugin.ObjFile, error) {
	b := bu.get()
	...
	if elfMagic == elf.ELFMAG {
		f, err := b.openELF(name, start, limit, offset, baseName)
		if err != nil {
...
func (b *binrep) openELF(name string, start, limit, offset uint64, baseName string) (plugin.ObjFile, error) {
	ef, err := elfOpen(name)
	if err != nil {
		return nil, fmt.Errorf("error parsing %s: %v", name, err)
	}
	...
	if strings.Contains(name, "vmlinux") || !pageAligned(start) || !pageAligned(limit) || !pageAligned(offset) {
		...
		// distinguised by the mapping name for the kernel image:
		// '[kernel.kallsyms]_text' or '[kernel.kallsyms]_stext', respectively.
		reloc := "_stext"
		if strings.Contains(baseName, "_text") {
			reloc = "_text"
		}
