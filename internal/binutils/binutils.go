...
func (bu *Binutils) Open(name string, start, limit, offset uint64, dso string) (plugin.ObjFile, error) {
	b := bu.get()
	...
	if elfMagic == elf.ELFMAG {
		f, err := b.openELF(name, start, limit, offset, dso)
		if err != nil {
...
func (b *binrep) openELF(name string, start, limit, offset uint64, dso string) (plugin.ObjFile, error) {
	ef, err := elfOpen(name)
	if err != nil {
		return nil, fmt.Errorf("error parsing %s: %v", name, err)
	}
