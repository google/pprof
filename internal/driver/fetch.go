...
mapping:
	...
	if len(p.Mapping) == 0 {
		// If there are no mappings, add a fake mapping to
		// attempt symbolization.  This is useful for some
		// profiles generated by the golang runtime, which do
		// not include any mappings. Symbolization with a fake
		// mapping will only be successful against a non-PIE
		// binary.
