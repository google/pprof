...
func convertPerfData(perfPath string, ui plugin.UI) (*os.File, error) {
	ui.Print(fmt.Sprintf(
		...
		perfPath))
	// Get a tempFile
	tempFilename, err := getNextTempFile(os.TempDir(), "pprof_", ".pb.gz")
	if err != nil {
		return nil, err
	}
	cmd := exec.Command("perf_to_profile", perfPath, tempFilename)
	if err := cmd.Run(); err != nil {
		return nil, fmt.Errorf("failed to convert perf.data file. Try github.com/google/perf_data_converter: %v", err)
	}

	// Read the profile generated by perf_to_profile
	profile, err := os.Open(tempFilename)
	if err != nil {
		return nil, fmt.Errorf("open profile for reading: %v", err)
	}
