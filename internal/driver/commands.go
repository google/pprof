...
func browsers() []string {
	return browsersForOS(runtime.GOOS)
}

// browsersForOS returns the commands to attempt for web visualization on goos.
func browsersForOS(goos string) []string {
	var cmds []string
	...
	case "windows":
		// Trailing space causes strings.Split(v, " ") to produce an empty argument
		// (""), which start takes as the window title. Without it start consumes a
		// path or URL containing a space as the title and never opens it.
