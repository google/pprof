...
var pprofVariables = variables{
	...
	"output": &variable{stringKind, []string{""}, "", helpText("Output filename for file-based outputs")},

	// Comparisons.
	"drop_negative": &variable{boolKind, []string{"f"}, "", helpText(
		"Ignore negative differences",
		"Do not show any locations with values <0.")},

	// Comparisons.
	"positive_percentages": &variable{boolKind, []string{"f"}, "", helpText(
	...
	"call_tree": &variable{boolKind, []string{"f"}, "", helpText(
		"Create a context-sensitive call tree",
		"Treat locations reached through different paths as separate.")},

	// Display options.
	"relative_percentages": &variable{boolKind, []string{"f"}, "", helpText(
	...
	"unit": &variable{stringKind, []string{"minimum"}, "", helpText(
		"Measurement units to display",
		"Scale the sample values to this unit.",
		" For time-based profiles, use seconds, milliseconds, nanoseconds, etc.",
		" For memory profiles, use megabytes, kilobytes, bytes, etc.",
		" auto will scale each value independently to the most natural unit.")},
	"compact_labels": &variable{boolKind, []string{"f"}, "", "Show minimal headers"},
	"source_path":    &variable{stringKind, []string{""}, "", "Search path for source files"},

	// Filtering options
	"nodecount": &variable{intKind, []string{"-1"}, "", helpText(
	...
	"nodefraction": &variable{floatKind, []string{"0.005"}, "", "Hide nodes below <f>*total"},
	"edgefraction": &variable{floatKind, []string{"0.001"}, "", "Hide edges below <f>*total"},
	"trim": &variable{boolKind, []string{"t"}, "", helpText(
		"Honor nodefraction/edgefraction/nodecount defaults",
		"Set to false to get the full profile, without any trimming.")},
	"focus": &variable{stringKind, []string{""}, "", helpText(
	...
	"hide": &variable{stringKind, []string{""}, "", helpText(
		...
		"Matching includes the function name, filename or object name.")},
	"show": &variable{stringKind, []string{""}, "", helpText(
	...
	"taghide": &variable{stringKind, []string{""}, "", helpText(
		"Skip tags matching this regexp",
		"Discard tags that match this regexp")},
	// Heap profile options
	"divide_by": &variable{floatKind, []string{"1"}, "", helpText(
	...
	"mean": &variable{boolKind, []string{"f"}, "", helpText(
	...
	"sample_index": &variable{stringKind, []string{""}, "", helpText(
		"Sample value to report (0-based index or name)",
		"Profiles contain multiple values per sample.",
		"Use sample_index=i to select the ith value (starting at 0).")},
	"normalize": &variable{boolKind, []string{"f"}, "", helpText(
		"Scales profile based on the base profile.")},

	// Data sorting criteria
	"flat": &variable{boolKind, []string{"t"}, "cumulative", helpText("Sort entries based on own weight")},
	"cum":  &variable{boolKind, []string{"f"}, "cumulative", helpText("Sort entries based on cumulative weight")},

	// Output granularity
	"functions": &variable{boolKind, []string{"t"}, "granularity", helpText(
		"Aggregate at the function level.",
		"Takes into account the filename/lineno where the function was defined.")},
	"files": &variable{boolKind, []string{"f"}, "granularity", "Aggregate at the file level."},
	"lines": &variable{boolKind, []string{"f"}, "granularity", "Aggregate at the source code line level."},
	"addresses": &variable{boolKind, []string{"f"}, "granularity", helpText(
		"Aggregate at the function level.",
		"Includes functions' addresses in the output.")},
	"noinlines": &variable{boolKind, []string{"f"}, "granularity", helpText(
		"Aggregate at the function level.",
		"Attributes inlined functions to their first out-of-line caller.")},
	"addressnoinlines": &variable{boolKind, []string{"f"}, "granularity", helpText(
...
func (v *variable) boolValue() bool {
	if len(v.value) != 1 {
		panic("expected exactly one value for bool")
	}
	b, err := stringToBool(v.value[0])
	if err != nil {
		panic("unexpected value " + v.value[0] + " for bool ")
	}
	return b
}
...
func (v *variable) intValue() int {
	if len(v.value) != 1 {
		panic("expected exactly one value for int")
	}
	i, err := strconv.Atoi(v.value[0])
	if err != nil {
		panic("unexpected value " + v.value[0] + " for int ")
	}
	return i
}
...
func (v *variable) floatValue() float64 {
	if len(v.value) != 1 {
		panic("expected exactly one value for float")
	}
	f, err := strconv.ParseFloat(v.value[0], 64)
	if err != nil {
		panic("unexpected value " + v.value[0] + " for float ")
	}
	return f
}
